...
/**
 * The Class PaymentClientDetails.
 */
public class PaymentClientDetails {
	...
	public String generateParentJson(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws JsonParseException, JsonMappingException, IOException {
		try {
			...
			parentJsonObject.put("echo",generateGetforPaymentClientDetailsDefaultGroupEchoJSON(workbook, "DT_EchoEntity", scenarioID, excelOperation));
			...
			Reporter.log("<b><font size=4 color=green> Get Payment Client Details </font></b>");
	...
	/**
	 * Generate getfor payment client details default group echo JSON.
	 *
	 * @param workbook the workbook
	 * @param sheetName the sheet name
	 * @param scenarioID the scenario ID
	 ...
	 * @return the object
	 */
	private Object generateGetforPaymentClientDetailsDefaultGroupEchoJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) {
	...
	private Object generateGetSymbolforDefaultGroupRequestJSON(XSSFWorkbook workbook, String sheetName, String scenarioID,ExcelOperation excelOperation) throws FileNotFoundException, IOException {
		...
		List<LinkedHashMap<String, String>>jsonMap=excelOperation.getScenarioData(workbook, sheetName, scenarioID);
		for(LinkedHashMap<String, String> json:jsonMap) {
			//here CancelSIPOrder Data Want from Sheet
			getGroupforLiveUserRequestJsonObject.put("data",generateGetSymbolforDefaultGroupDataJSON(workbook, "DT_PaymentClientDetails", scenarioID, excelOperation));
		...
		JSONObject getGroupforLiveUserRequestJsonObject=new JSONObject();
			...
			for(LinkedHashMap<String, String> jsons:jsonMaps) 
			{
				for(LinkedHashMap<String, String> jsons1:jsonMaps1)
				{
					...
					getGroupforLiveUserRequestJsonObject.put("TransactionPassword", jsons.get("TransactionPassword"));		
	 ...
	 */
	public void GetPaymentClientDetails(XSSFWorkbook workbook,String sheetName,ExcelOperation excelOperation,String scenarioID) throws Exception {
		try {
			String uri=FrameworkServices.getConfigProperties().getProperty("ApplicationURI")+ServiceEndpoint.GetPayment_ClientDetails.getUrl();
			...
			if(infoid.equalsIgnoreCase("0"))
			{
				    String token=((JsonObject) ((JsonObject)(JsonObject) ((JsonObject) jsonObject.get("response")).get("data")).get("paymentDetails")).get("token").toString().replace("\"", "");
				    System.out.println("MD Anzaf SipInstructionID Value:-"+token);
				    
				    XSSFSheet sheet=workbook.getSheet("DT_paymentGatewayGenerator");
					...
					for(int j=0;j<sheet.getPhysicalNumberOfRows();j++)
					{
						...
						if(cell.getStringCellValue().equalsIgnoreCase(scenarioID)) {
							excelOperation.updateDataInExcelForInstructionID(workbook, "DT_paymentGatewayGenerator",scenarioID,"token", token);
							System.out.println("Updated Data token To DT_paymentGatewayGenerator ");
						}
					}
				    Assert.assertEquals("0", infoid);
				...
				Assert.assertEquals("ENW00103", infoid, "Incorrect Parameters");
			}	
		}
		catch(Exception e) {
			e.printStackTrace();	
